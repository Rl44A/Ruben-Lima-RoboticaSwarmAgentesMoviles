...
char *token;	// almacenar mensaje recibido
char *token2;
int bandera = 0;
...
void *rpzw(void *ptr){
  ...
  //server_addr.sin_addr.s_addr = inet_addr("192.168.5.221");
  server_addr.sin_addr.s_addr = inet_addr("192.168.50.167");
  ...
  while (1) {
    //Se crea la conexion y se acepta el cliente
   

    //se limpia el buffer antes de recibir el mensaje
    //memset(buffer, '\0', sizeof(buffer));
    recv(client_fd, buffer, 13, 0);
    printf("recibido %s\n",buffer);
    char rec[2];
		char *token;
		int i=0;
...
//Rec ya tiene el valor de X y Y --- Hacemos unos tramites y luego los mandamos de regreso 
        ...
        //best_global[0] = posicion_robot_X;
		//best_global[1] = posicion_robot_Y;
		theta_o = 45;
    printf("Coordenadas actuales agenteRPIZW %d son: %f, %f, %f.\n", num_agente, posicion_robot_X, posicion_robot_Y, theta_o);	
    PSO();
    sprintf(Totalbuf, "%f5,%f5",new_position[0],new_position[1]);
    //printf("Coordenadas a mandar %f, %f.\n", new_position[0],new_position[1]);
    send(client_fd, Totalbuf, strlen(Totalbuf), 0);
    printf("Coordenadas a mandar %f, %f.\n", new_position[0],new_position[1]);
		//printf("Coordenadas actuales agente son: %f, %f, %f.\n", posicion_robot_X, posicion_robot_Y, rad);	
...
int main() {
  ...
  // Variables and structures
    ...
    FILE *file;
    strcpy(IP_broadcast, "192.168.50.255"); 				// variables para multihilos
    ...
    pthread_create(&thread_rec2, NULL, rpzw, NULL); 
    ...
    printf("Raspberry Zero W conectada");
